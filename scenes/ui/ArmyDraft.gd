...
func _build_rune_circle() -> void:
	...
	rune_circle.position = Vector2(512, 480)
...
func _build_ui() -> void:
	...
	title_container.offset_bottom = 150
	...
	title_label.text = "Assemble Your Swarm"
	...
	budget_label.add_theme_color_override("font_color", Color(0.85, 0.78, 0.55, 0.9))
	budget_label.add_theme_font_size_override("font_size", 18)
	budget_label.mouse_filter = Control.MOUSE_FILTER_IGNORE
	title_container.add_child(budget_label)

	# ── Main content area ──
	var content_container := VBoxContainer.new()
	content_container.set_anchors_preset(Control.PRESET_CENTER)
	content_container.offset_left = -380
	content_container.offset_right = 380
	content_container.offset_top = -80
	content_container.offset_bottom = 120
	...
	ui_root.add_child(content_container)

	# Budget bar
	_setup_budget_bar()
	content_container.add_child(budget_bar)
	...
	content_container.add_child(rows_container)
	...
	enter_button.custom_minimum_size = Vector2(280, 48)
...
func _setup_budget_bar() -> void:
	...
	budget_bar.custom_minimum_size = Vector2(400, 12)
...
func _build_unit_rows() -> void:
	...
	for unit_type in ALL_UNIT_TYPES:
		...
		if row_index > 0:
			...
			sep.custom_minimum_size = Vector2(750, 1)
			sep.color = Color(0.5, 0.5, 0.5, 0.2)
